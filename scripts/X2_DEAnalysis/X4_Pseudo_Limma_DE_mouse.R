...
out_dir <- 'data/DE_out/Pseudo_Limma_mouse/'
...
species_names <- c('mouse') #unique(species)
...
print("mm10-Gpx3" %in% rownames(x))

# OLD Filtering genes #
cutoff <- 2 #5 is the original value used here
drop <- which(apply(cpm(x), 1, max) < cutoff)
x <- x[-drop,]

#cutoff <- 0 # - original here
...
genes_bool <- detected_genes & mouse_genes
x <- x[genes_bool,]
dim(x) # number of genes left
# 13700             # OLD 13689 remaining
...
tfit <- treat(vfit, lfc=.15)# was originally .1
dt <- decideTests(tfit)
...
##### Saving the results #####
file_name <- paste(out_dir, 'de_results_tfit_', 'mouse', suffix, '.rds', sep='')
...
# tfit <- readRDS(file_name)
save_results(tfit, out_dir, 'mouse', suffix)


# NOTE that I did also try additional paramters, such as:
#   5 for the maximum expression observed for the gene cutoff, and 
#   different treat criterion cutoffs; overall suggested no DE genes 
#   since is a clear batch effect between the samples. Think that
#   filtering out these genes was the right thing to do. 
